...
import fr.yvz.stopandgo.billing.BillingManager
...
class PlayerViewModel(application: Application) : AndroidViewModel(application) {
    ...
    val billing: BillingManager = BillingManager.getInstance(context)

    // Free tier limit (matches iOS)
    val freeTrackLimit = 10
    ...
    var playlistName by mutableStateOf(prefs.getString("playlistName", "My first playlist") ?: "My first playlist")
        private set
    var showVolumeView by mutableStateOf(prefs.getBoolean("volumeSlider", false))
    ...
    var autoLock by mutableStateOf(prefs.getBoolean("autolock", false))
    ...
    init {
        ...
        // Connect billing and observe premium status
        billing.connect()
        viewModelScope.launch {
            billing.isPremium.collect { value ->
                isPremium = value
            }
        }
    }
    ...
    /**
     * Returns true if the import was truncated due to the free tier limit
     * (caller can then display the upgrade screen).
     */
    fun addFiles(uris: List<Uri>): Boolean {
        ...
        if (!isPremium) {
            val remaining = (freeTrackLimit - playlistSongs.size).coerceAtLeast(0)
            if (uris.size > remaining) {
                toImport = uris.take(remaining)
                didHitLimit = true
            }
        }

        if (toImport.isEmpty()) return true

        isImporting = true
        viewModelScope.launch(kotlinx.coroutines.Dispatchers.IO) {
            engine.addFilesFromDocumentPicker(toImport)
    ...
    fun setAutoLockPref(value: Boolean) {
        autoLock = value
        prefs.edit().putBoolean("autolock", value).apply()
    }
