...
class PlaylistData private constructor(private val context: Context) {
    ...
    fun addFileFromUri(sourceUri: Uri): Boolean {
        ...
        if (title.isNullOrEmpty()) {
            title = queryDisplayName(sourceUri)?.substringBeforeLast('.')
                ?: sourceUri.lastPathSegment?.substringAfterLast('/')?.substringBeforeLast('.')
                ?: "Unknown"
        }
    ...
    /** Queries the document's display name (e.g. "song.mp3") from the content resolver. */
    ...
    fun getSongs(): List<Song> {
        return currentPlaylist?.songList?.map { container ->
            ...
            // Stable ID derived from the file path so the same song always
            // produces the same id across reloads (required for drag-to-reorder
            // and Compose item keys).
            val stableId = container.localAudioPath
                ?: ("${container.title ?: ""}|${container.artist ?: ""}|${container.album ?: ""}")
            Song(
                id = stableId,
